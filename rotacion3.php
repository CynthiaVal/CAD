<?php
...

  {
      ...
      for($i=0;$i<$num0;$i++)
      {
        $poligono2[$i][0]=$poligono[$i][0]-$px;
        $poligono2[$i][1]=$poligono[$i][1]-$py;
      }

      for($i=0;$i<$num0;$i++)
      {
        $poligono3[$i][0]=$poligono2[$i][0]*cos(deg2rad($alfa))-$poligono2[$i][1]*sin(deg2rad($alfa));
        $poligono3[$i][1]=$poligono2[$i][1]*cos(deg2rad($alfa))+$poligono2[$i][0]*sin(deg2rad($alfa));    
      }

      for($i=0;$i<$num0;$i++)
      {
        $poligono4[$i][0]=$poligono3[$i][0]+$px;
        $poligono4[$i][1]=$poligono3[$i][1]+$py;     
      }
      ...
      for($i=0;$i<$num0;$i++)
      {
         if($i==$num0-1)
         {
          imageline($gd, $poligono4[$i][0],$poligono4[$i][1], $poligono4[0][0], $poligono4[0][1], $azul); 
         }else
         {
         imageline($gd, $poligono4[$i][0],$poligono4[$i][1], $poligono4[$i+1][0], $poligono4[$i+1][1], $azul);
         }
